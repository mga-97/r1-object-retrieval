...
from mdetr_python_code.utils.common import shift_dim
...
class MultiHeadAttention(nn.Module):
    ...
    Args:
        ...
        attn_module (nn.Module): Module of attention mechanism to use. Default is ``SelfAttention``.
            See :class:`~code.modules.layers.attention.SelfAttention` for API details.
...
def scaled_dot_product_attention(
    ...
    Args:
        ...
        k (Tensor): Key of shape ``(b, h, d1', ...., dn', dim_qk)`` or ``(b, h, seq_len', dim_qk)`` where
            ...
            see :class:`~code.modules.layers.attention.MultiHeadAttention`.
